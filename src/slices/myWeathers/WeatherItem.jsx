...
export default function WeatherItem({position, id}) {
  ...
  if(isPending) return <div className="flex justify-center items-center p-8 bg-white/20 dark:bg-[#252525]/60 shadow-2xl rounded-md" >
    <Spinner size={10} />
  </div>
  ...
  return (
    <div className="relative flex flex-col md:flex-row gap-2 p-6 min-h-[380px] bg-white/20 dark:bg-[#252525]/60 shadow-2xl rounded-md" >

      <DeleteWeatherBtn id={id} /> 

      <div className="flex flex-col gap-4 pr-4 mr-2 w-full md:w-5/12 min-[960px]:w-6/12 md:border-r-[1px] border-black/20 dark:border-white/20" >

        <div className="flex flex-col gap-1" >

          <Link to={`/dashboard/${position.lat} ${position.lon}`} 
              className="font-bold text-3xl min-[550px]:text-4xl dark:text-white"
          > 
              {data?.name}, {data?.sys?.country}
          </Link>

          <p className="font-light text-lg min-[550px]:text-xl dark:text-white">{ isPendingTime ? "Loading..." :  formattedDate}</p>
        ...
        <div className="flex md:justify-between md:flex-col min-[960px]:flex-row h-full" >
          ...
          <div className="flex flex-col justify-center gap-2 min-[960px]:w-6/12" >
            <p className="text-3xl min-[550px]:text-7xl text-center dark:text-white" >{temperature.toFixed(0)}°{isCelcius ? "C" : "F"}</p>
            <p className="text-lg min-[550px]:text-2xl font-medium text-center dark:text-white" >{data?.weather[0]?.description}</p>
          </div>
